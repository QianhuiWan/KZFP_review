...
# multiple genes + fixed repeat design + fixed gene universe
# + stable hit summary + save full results from all repeats
# normal Epi vs tumor EpiT
# ============================================================
# All KO genes were evaluated using the same repeated subsampling
# design for normal and tumor epithelial cells, ensuring that
# between-gene differences were not driven by inconsistent cell
...
## all KZFPs
human_KZFPs_age_R2 <- read_tsv(
  file = "/net/nfs-irwrsrchnas01/labs/dschones/bioresearch/qianhui/hg38_2024/KZFPs/KZFP_evolutional_age_R2.tsv",
  show_col_types = FALSE
)

## primate-specific KZFPs
primate_specific_KZFPs_R2 <- read_tsv(
  file = "/net/nfs-irwrsrchnas01/labs/dschones/bioresearch/qianhui/hg38_2024/KZFPs/KZFP_evolutional_primate_specific_R2.tsv",
  show_col_types = FALSE
)
...
outdir <- "/net/nfs-irwrsrchnas01/labs/dschones/bioresearch/qianhui/projects/kzfp/ko_output/scTenifoldKnk_multiKO_fixedDesign_final"
...
# gene filtering for repeat-specific cleanup
min_cells_expr <- 10

# fixed gene universe
min_cells_expr_global <- 20   # testing: 20 ; formal: 10
top_n_genes_fixed <- 3000     # testing: 3000 ; formal: 5000
force_keep_ko_genes <- TRUE

# scTenifoldKnk parameters
qc_minLSize    <- 0
qc_minCells    <- 25
nc_lambda      <- 0.1
nc_nNet        <- 3           # testing: 3 ; formal: 10
nc_nCells      <- 200         # testing: 200 ; formal: 500
nc_nComp       <- 3
nc_scaleScores <- TRUE
nc_symmetric   <- FALSE
nc_q           <- 0.9
...
top_n_per_run      <- 50      # only for consensus summary, not for full-result saving
...
extract_rank_table <- function(res_obj) {
  ...
  if (is.list(res_obj)) {
    ...
    if (length(hit) > 0) {
      return(extract_rank_table(res_obj[[hit[1]]]))
    }
...
run_one_knk <- function(mat_sparse, gKO_gene, seed, group_name, repeat_id,
                        ...
                        td_K, td_maxIter, td_maxError, ma_nDim,
                        top_n_per_run, gene_outdir) {
  ...
  rank_df_full <- extract_rank_table(res) %>%
    mutate(
      KO_gene = gKO_gene,
      group = group_name,
      repeat_id = repeat_id,
  ...
  # save full ranked result for this repeat/group
  fwrite(
    rank_df_full,
    file = file.path(gene_outdir, paste0(group_name, "_repeat", repeat_id, "_", gKO_gene, "_full.tsv")),
    sep = "\t"
  )
  
  # save top genes for stable summary
  rank_df_top <- rank_df_full %>%
    slice_head(n = min(top_n_per_run, n()))
...
# use NAME because your matrix colnames match NAME
...
# ============================================================
# 3.5 fixed gene universe
# ============================================================
message("\nBuilding fixed gene universe ...")

cells_all_for_universe <- union(normal_cells_all, tumor_cells_all)
mat_all_for_universe <- mat[, cells_all_for_universe, drop = FALSE]

# detection filter on all epithelial cells
keep_global <- Matrix::rowSums(mat_all_for_universe > 0) >= min_cells_expr_global
mat_all_for_universe_filt <- mat_all_for_universe[keep_global, , drop = FALSE]

message("After global detection filtering: ", nrow(mat_all_for_universe_filt), " genes")

# choose top variable genes from all epithelial cells
mat_all_dense <- as.matrix(mat_all_for_universe_filt)
gene_var <- apply(mat_all_dense, 1, var)

fixed_gene_set <- names(sort(gene_var, decreasing = TRUE))[1:min(top_n_genes_fixed, length(gene_var))]

if (force_keep_ko_genes) {
  fixed_gene_set <- union(fixed_gene_set, intersect(gKO_genes, rownames(mat)))
}

fixed_gene_set <- unique(fixed_gene_set)

message("Final fixed gene universe size: ", length(fixed_gene_set))

write.table(
  data.frame(gene = fixed_gene_set),
  file = file.path(outdir, "fixed_gene_universe.tsv"),
  sep = "\t", quote = FALSE, row.names = FALSE
)

rm(mat_all_dense, mat_all_for_universe, mat_all_for_universe_filt, gene_var)
gc()
...
for (i in seq_len(n_repeats)) {
  ...
  # enforce fixed gene universe
  common_genes <- intersect(
    fixed_gene_set,
    intersect(rownames(mat_normal), rownames(mat_tumor))
  )
  
  mat_normal <- mat_normal[common_genes, , drop = FALSE]
  mat_tumor  <- mat_tumor[common_genes,  , drop = FALSE]
...
# ============================================================
# 6. run all KO genes using fixed repeat design + fixed gene universe
# ============================================================
...
for (gKO_gene in gKO_genes) {
  ...
  for (i in seq_len(n_repeats)) {
    ...
    common_genes <- repeat_mats[[i]]$common_genes
    
    if (!(gKO_gene %in% common_genes)) {
      warning("Skipping repeat ", i, " for ", gKO_gene,
              ": gene not retained after filtering / fixed universe in this repeat.")
      next
    }
    ...
    all_rank_tables[[paste0("normal_", i)]] <- rank_normal$top
    all_rank_tables[[paste0("tumor_", i)]]  <- rank_tumor$top
    
    all_rank_tables_full[[paste0("normal_", i)]] <- rank_normal$full
    all_rank_tables_full[[paste0("tumor_", i)]]  <- rank_tumor$full
  }
  ...
  rank_all_full <- bind_rows(all_rank_tables_full)
  ...
  fwrite(
    rank_all_full,
    file = file.path(gene_outdir, paste0("all_repeats_ranked_full_", gKO_gene, ".tsv")),
    sep = "\t"
  )
